 ...
 */
function bootslightning_search_title() {
	if ( isset( $_GET['s'] ) ) {
		$search_term = sanitize_text_field( wp_unslash( $_GET['s'] ) );
		/* translators: %s: Search term. */
		return isset( $search_term ) ? sprintf( esc_html__( 'Search results for "%s"', 'bootslightning' ), esc_html( $search_term ) ) : __( 'Search results', 'bootslightning' );
	}
}
 ...
 */
function bootslightning_privacy() {
 ...
 * @return void
 */
function bootslightning_register_block_patterns() {
	...
	register_block_pattern_category(
		'images',
		array( 'label' => esc_html__( 'Images', 'bootslightning' ) )
	);

}
add_action( 'init', 'bootslightning_register_block_patterns', 9 );
 ...
 * @return void
 */
function bootslightning_unregister_patterns() {
	unregister_block_pattern( 'core/query-small-posts' );
	unregister_block_pattern( 'core/query-large-title-posts' );
	unregister_block_pattern( 'core/query-offset-posts' );
	unregister_block_pattern_category( 'header' );
}
add_action( 'init', 'bootslightning_unregister_patterns', 10 );
